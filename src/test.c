...
#include "calculator/s21_calc.h"

START_TEST(test_smart_calc_1) {
  char input[255] = "3+4*2/(1-5)^2";
  double result = evaluateExpression(input, 0);
...
START_TEST(test_smart_calc_10) {
  ...
  double result = evaluateExpression(input, x);
...
START_TEST(test_smart_calc_31) {
  double xMax = 10.0, xMin = 0.0, yMax = 5.0, yMin = -5.0;
  int result = plotValid(xMax, xMin, yMax, yMin);
  ck_assert_int_eq(result, SUCCESS);
}
END_TEST

START_TEST(test_smart_calc_32) {
  double xMax = 0.0, xMin = 10.0, yMax = 5.0, yMin = -5.0;
  int result = plotValid(xMax, xMin, yMax, yMin);
